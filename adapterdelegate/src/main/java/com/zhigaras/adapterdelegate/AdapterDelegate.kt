...
abstract class AdapterDelegate<M : ListItem, in VH : ViewHolderDelegate<M>> {
    ...
    fun bindViewHolder(item: M, viewHolder: VH, position: Int) {
        viewHolder.bind(item, position)
    }

    fun bindViewHolder(viewHolder: VH, payloads: List<Payload<ViewBinding>>, position: Int) {
        payloads.forEach { viewHolder.bind(it, position) }
    }
}
